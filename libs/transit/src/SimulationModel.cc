...
SimulationModel::SimulationModel(IController& controller)
  ...
  entityFactory.AddFactory(new FoodTruckFactory());
}
...
IEntity* SimulationModel::createEntity(JsonObject& entity) {
  ...
  if (type == "foodtruck") this->foodTruck = static_cast<FoodTruck*>(myNewEntity);
  if (type == "human") this->foodTruck->addHuman(myNewEntity);
...
bool SimulationModel::isNearby(const Vector3& pos1, const Vector3& pos2) const {
